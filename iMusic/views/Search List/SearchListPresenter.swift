...
extension SearchListPresenter {
    ...
    private func getTracks(_ search: String? = nil, showProgress: Bool) {
        view?.showProgress(showProgress, status: "Cargando tracks")
        ...
        interactor.getTracksList(search: search) { [weak self] (artists, success, error) in
            ...
            if let error = error {
                self.view?.showMessageWith(title: "Oops... 🧐", message: error.localizedDescription, actionTitle: "Aceptar")
                return
            }
            
            if !success {
                self.view?.showMessageWith(title: "Oops... 🧐", message: "Algo raro esta pasando. Porfavor intente nuevamente", actionTitle: "Aceptar")
                return
            }
        }
    }
