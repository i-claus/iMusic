...
extension SearchListViewController {
    ...
    private func configureSubviews() {
        ...
        sortButton.setTitle("Filtro", for: .normal)
        ...
        noResultsLabel.text = "No hay resultados. Inténtalo de nuevo"
    ...
    private func configureNavigationBar() {
        ...
        customTitleView.setTitle("iMusic Bci")
...
extension SearchListViewController {
    ...
    private func showSortOptions() {
        let alertController = UIAlertController(
            title: "Ordenar opciones por resultado",
            message: nil,
            preferredStyle: .actionSheet)
        
        alertController.addAction(UIAlertAction(title: "Por longitud de la canción", style: .default, handler: { [weak self] (action) in
            guard let `self` = self else { return }
            self.sortResultsBy(.length)
        }))
        
        alertController.addAction(UIAlertAction(title: "Por genero", style: .default, handler: { [weak self] (action) in
            guard let `self` = self else { return }
            self.sortResultsBy(.genre)
        }))
        
        alertController.addAction(UIAlertAction(title: "Por precio", style: .default, handler: { [weak self] (action) in
            guard let `self` = self else { return }
